...
import com.github.mrgrtt.ioc.bean_reader_test.BeanD;
...
class AnnotationBeanDefinitionReaderTest {
    ...
    public void test() throws IOException {
        Class<?>[] beanClasses = new Class[] {BeanA.class, BeanB.class, BeanC.class, BeanD.class};
        ...
        for (Class<?> c: beanClasses) {
            ...
            if (c == BeanD.class) {
                assertEquals(df.getBeanName(), "beanD");
                assertEquals(df.getDependencyFields().size(), 0);
                assertEquals(df.getCreatorParams().length, 0);
                assertEquals(df.getScope(), Scope.PROTOTYPE);
            }
        }
    }
